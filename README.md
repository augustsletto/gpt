...
You can control how much text is generated by changing the `MAX_NEW_TOKENS` parameter at the top of `bigram.py`:

```python
MAX_NEW_TOKENS = 500  # increase for longer output, decrease for shorter
```

Higher values produce more text; lower values produce less.
